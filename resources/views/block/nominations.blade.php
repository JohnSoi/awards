<section class="section section-content section-themes black-block new-main">
    <div class="container">
        <div class="section-themes__inner">
            <div class="border-top-white"></div>
            <h2 class="section-title mb-0 wow fadeInUp" data-wow-duration="0.8s" data-wow-delay="0.2s"
                data-wow-offset="0">{{__('main.whoCanNom_mane')}}</h2>
        </div>
    </div>
    <div cursor-status class="section-themes__content wow fadeIn" data-wow-duration="0.8s" data-wow-delay="0.1s"
         data-wow-offset="0">
        <div class="swiper-container swiper-step wow fadeIn swiper-container-initialized swiper-container-horizontal swiper-container-pointer-events">
            <div class="swiper-wrapper">
                <div class="swiper-slide">
                    <div class="card card-themes white" data-white>
                        <div class="card-themes__title">
                            <div class="bg_point bg-black"></div>
                            <span class="text">{{__('main.whoCanNom_gildia1')}}<br>
                                Russian Creative Week<br>
                                {{__('main.whoCanNom_gildia2')}}<br>{{__('main.whoCanNom_gildia3')}}</span>
                        </div>
                        <div class="card-themes__content">
                            <div class="list-type">
                                <div class="list-type__item">
                                    <div class="icon">
                                        <svg width="35" height="8" viewBox="0 0 35 8" fill="none"
                                             xmlns="http://www.w3.org/2000/svg">
                                            <path d="M34.8536 4.35355C35.0488 4.15829 35.0488 3.84171 34.8536 3.64645L31.6716 0.464466C31.4763 0.269204 31.1597 0.269204 30.9645 0.464466C30.7692 0.659728 30.7692 0.976311 30.9645 1.17157L33.7929 4L30.9645 6.82843C30.7692 7.02369 30.7692 7.34027 30.9645 7.53553C31.1597 7.7308 31.4763 7.7308 31.6716 7.53553L34.8536 4.35355ZM0.5 4.5H34.5V3.5H0.5V4.5Z"
                                                  fill="#222222"/>
                                        </svg>
                                    </div>
                                    <div class="text">{{__('main.whoCanNom_gildia4')}}<br>
                                        {{__('main.whoCanNom_gildia5')}}<br>
                                        {{__('main.whoCanNom_gildia6')}}
                                    </div>
                                </div>
                            </div>
                        </div>
                    </div>
                </div>
                <div class="swiper-slide">
                    <div class="card card-themes black">
                        <div class="card-themes__title">
                            <div class="bg_point bg-pink"></div>
                            <span class="text">{{__('main.whoCanNom_ProfOrg1')}} <br>{{__('main.whoCanNom_ProfOrg2')}} <br>{{__('main.whoCanNom_ProfOrg3')}} <br>{{__('main.whoCanNom_ProfOrg4')}}</span>
                        </div>
                        <div class="card-themes__content">
                            <div class="list-type">
                                <div class="list-type__item">
                                    <div class="icon">
                                        <svg width="35" height="8" viewBox="0 0 35 8" fill="none"
                                             xmlns="http://www.w3.org/2000/svg">
                                            <path d="M34.8536 4.35355C35.0488 4.15829 35.0488 3.84171 34.8536 3.64645L31.6716 0.464466C31.4763 0.269204 31.1597 0.269204 30.9645 0.464466C30.7692 0.659728 30.7692 0.976311 30.9645 1.17157L33.7929 4L30.9645 6.82843C30.7692 7.02369 30.7692 7.34027 30.9645 7.53553C31.1597 7.7308 31.4763 7.7308 31.6716 7.53553L34.8536 4.35355ZM0.5 4.5H34.5V3.5H0.5V4.5Z"
                                                  fill="#222222"/>
                                        </svg>
                                    </div>
                                    <div class="text">{{__('main.whoCanNom_ProfOrg5')}}<br>
                                        {{__('main.whoCanNom_ProfOrg6')}}<br>
                                        {{__('main.whoCanNom_ProfOrg7')}}
                                    </div>
                                </div>
                            </div>
                        </div>
                    </div>
                </div>
                <div class="swiper-slide">
                    <div class="card card-themes white" data-white>
                        <div class="card-themes__title">
                            <div class="bg_point bg-black"></div>
                            <span class="text">{{__('main.whoCanNom_Lau1')}}</span>
                        </div>
                        <div class="card-themes__content">
                            <div class="list-type">
                                <div class="list-type__item">
                                    <div class="icon">
                                        <svg width="35" height="8" viewBox="0 0 35 8" fill="none"
                                             xmlns="http://www.w3.org/2000/svg">
                                            <path d="M34.8536 4.35355C35.0488 4.15829 35.0488 3.84171 34.8536 3.64645L31.6716 0.464466C31.4763 0.269204 31.1597 0.269204 30.9645 0.464466C30.7692 0.659728 30.7692 0.976311 30.9645 1.17157L33.7929 4L30.9645 6.82843C30.7692 7.02369 30.7692 7.34027 30.9645 7.53553C31.1597 7.7308 31.4763 7.7308 31.6716 7.53553L34.8536 4.35355ZM0.5 4.5H34.5V3.5H0.5V4.5Z"
                                                  fill="#222222"/>
                                        </svg>
                                    </div>
                                    <div class="text">{{__('main.whoCanNom_Lau2')}}<br>
                                        {{__('main.whoCanNom_Lau3')}}<br>
                                        {{__('main.whoCanNom_Lau4')}}
                                    </div>
                                </div>
                            </div>
                        </div>
                    </div>
                </div>
                <div class="swiper-slide">
                    <div class="card card-themes black">
                        <div class="card-themes__title">
                            <div class="bg_point bg-pink"></div>
                            <span class="text">{{__('main.whoCanNom_RegOr1')}}</span>
                        </div>
                        <div class="card-themes__content">
                            <div class="list-type">
                                <div class="list-type__item">
                                    <div class="icon">
                                        <svg width="35" height="8" viewBox="0 0 35 8" fill="none"
                                             xmlns="http://www.w3.org/2000/svg">
                                            <path d="M34.8536 4.35355C35.0488 4.15829 35.0488 3.84171 34.8536 3.64645L31.6716 0.464466C31.4763 0.269204 31.1597 0.269204 30.9645 0.464466C30.7692 0.659728 30.7692 0.976311 30.9645 1.17157L33.7929 4L30.9645 6.82843C30.7692 7.02369 30.7692 7.34027 30.9645 7.53553C31.1597 7.7308 31.4763 7.7308 31.6716 7.53553L34.8536 4.35355ZM0.5 4.5H34.5V3.5H0.5V4.5Z"
                                                  fill="#222222"/>
                                        </svg>
                                    </div>
                                    <div class="text">{{__('main.whoCanNom_RegOr2')}}<br>
                                        {{__('main.whoCanNom_RegOr3')}}<br>
                                        {{__('main.whoCanNom_RegOr4')}}
                                    </div>
                                </div>
                            </div>
                        </div>
                    </div>
                </div>
                <div class="swiper-slide">
                    <div class="card card-themes white" data-white>
                        <div class="card-themes__title">
                            <div class="bg_point bg-black"></div>
                            <span class="text">{{__('main.whoCanNom_Ind1')}}</span>
                        </div>
                        <div class="card-themes__content">
                            <div class="list-type">
                                <div class="list-type__item">
                                    <div class="icon">
                                        <svg width="35" height="8" viewBox="0 0 35 8" fill="none"
                                             xmlns="http://www.w3.org/2000/svg">
                                            <path d="M34.8536 4.35355C35.0488 4.15829 35.0488 3.84171 34.8536 3.64645L31.6716 0.464466C31.4763 0.269204 31.1597 0.269204 30.9645 0.464466C30.7692 0.659728 30.7692 0.976311 30.9645 1.17157L33.7929 4L30.9645 6.82843C30.7692 7.02369 30.7692 7.34027 30.9645 7.53553C31.1597 7.7308 31.4763 7.7308 31.6716 7.53553L34.8536 4.35355ZM0.5 4.5H34.5V3.5H0.5V4.5Z"
                                                  fill="#222222"/>
                                        </svg>
                                    </div>
                                    <div class="text">{{__('main.whoCanNom_Ind2')}}<br>
                                        {{__('main.whoCanNom_Ind3')}}<br>
                                        {{__('main.whoCanNom_Ind4')}}
                                    </div>
                                </div>
                            </div>
                        </div>
                    </div>
                </div>
            </div>
            <div class="slider-navigation pl-sm-0 pl-md-0 wow fadeIn" data-wow-duration="0.8s" data-wow-delay="0.2s"
                 data-wow-offset="0">
                <div class="arrows-nav">
                    <div class="arrows-nav__left">
                        <svg width="46" height="46" viewBox="0 0 46 46" fill="none" xmlns="http://www.w3.org/2000/svg">
                            <circle cx="23" cy="23" r="22.5" transform="rotate(-180 23 23)" stroke="#FBFBFB"/>
                            <path d="M11.6464 22.6464C11.4512 22.8417 11.4512 23.1583 11.6464 23.3536L14.8284 26.5355C15.0237 26.7308 15.3403 26.7308 15.5355 26.5355C15.7308 26.3403 15.7308 26.0237 15.5355 25.8284L12.7071 23L15.5355 20.1716C15.7308 19.9763 15.7308 19.6597 15.5355 19.4645C15.3403 19.2692 15.0237 19.2692 14.8284 19.4645L11.6464 22.6464ZM34 22.5L12 22.5L12 23.5L34 23.5L34 22.5Z"
                                  fill="#FBFBFB"/>
                        </svg>
                    </div>
                    <div class="arrows-nav__right">
                        <svg width="46" height="46" viewBox="0 0 46 46" fill="none" xmlns="http://www.w3.org/2000/svg">
                            <circle cx="23" cy="23" r="22.5" stroke="#FBFBFB"/>
                            <path d="M34.3536 23.3536C34.5488 23.1583 34.5488 22.8417 34.3536 22.6464L31.1716 19.4645C30.9763 19.2692 30.6597 19.2692 30.4645 19.4645C30.2692 19.6597 30.2692 19.9763 30.4645 20.1716L33.2929 23L30.4645 25.8284C30.2692 26.0237 30.2692 26.3403 30.4645 26.5355C30.6597 26.7308 30.9763 26.7308 31.1716 26.5355L34.3536 23.3536ZM12 23.5H34V22.5H12V23.5Z"
                                  fill="#FBFBFB"/>
                        </svg>
                    </div>
                </div>
                <div class="counter">
                    <span data-active-count>1</span>
                    /
                    <span data-all-count></span>
                </div>
            </div>
        </div>
    </div>
</section>

<section class="section section-content section-mechanics">
    <div class="container" id="criteria">
        <div class="section-mechanics__main">
            <div class="section-mechanics__main-title wow fadeInUp" data-wow-duration="0.8s" data-wow-delay="0.2s"
                 data-wow-offset="30">
                {{ __('main.grade_mane') }}:
            </div>
        </div>
        <div class="section-mechanics__stages-wrapper">
            <div class="section-mechanics__stages-stage__wrapper wow fadeIn" data-wow-duration="1.1s"
                 data-wow-delay="0.2s"
                 data-wow-offset="30">
                <div class="section-mechanics__stages-stage__number">1.</div>
                <div class="section-mechanics__stages-stage__title">{{ __('main.grade_media1') }}</div>
            </div>
            <div class="section-mechanics__stages-stage__wrapper wow fadeIn" data-wow-duration="1.2s"
                 data-wow-delay="0.2s"
                 data-wow-offset="30">
                <div class="section-mechanics__stages-stage__number">2.</div>
                <div class="section-mechanics__stages-stage__title">{{ __('main.grade_award2021') }}</div>
            </div>
            <div class="section-mechanics__stages-stage__wrapper wow fadeIn" data-wow-duration="1.3s"
                 data-wow-delay="0.2s"
                 data-wow-offset="30">
                <div class="section-mechanics__stages-stage__number">3.</div>
                <div class="section-mechanics__stages-stage__title">{{ __('main.grade_unique1') }}</div>
            </div>
        </div>
    </div>
</section>

<section class="section section-content section-mechanics">
    <div class="container" id="criteria">
        <div class="section-mechanics__main">
            <div class="section-mechanics__main-title wow fadeInUp" data-wow-duration="0.8s"
                 data-wow-delay="0.2s"
                 data-wow-offset="30">
                {{ __('main.grade_mane_man') }}:
            </div>
        </div>
        <div class="section-mechanics__stages-wrapper">
            <div class="section-mechanics__stages-stage__wrapper wow fadeIn" data-wow-duration="1s"
                 data-wow-delay="0.2s"
                 data-wow-offset="30">
                <div class="section-mechanics__stages-stage__number">1.</div>
                <div class="section-mechanics__stages-stage__title">
                    {{ __('main.grade_unique2') }}
                </div>
            </div>
            <div class="section-mechanics__stages-stage__wrapper wow fadeIn" data-wow-duration="1.1s"
                 data-wow-delay="0.2s"
                 data-wow-offset="30">
                <div class="section-mechanics__stages-stage__number">2.</div>
                <div class="section-mechanics__stages-stage__title">{{ __('main.grade_media1') }}</div>
            </div>
            <div class="section-mechanics__stages-stage__wrapper wow fadeIn" data-wow-duration="1.2s"
                 data-wow-delay="0.2s"
                 data-wow-offset="30">
                <div class="section-mechanics__stages-stage__number">3.</div>
                <div class="section-mechanics__stages-stage__title">{{ __('main.grade_award2021') }}</div>
            </div>
        </div>
    </div>
</section>

@if( Request::url() === Route('index') )
    <div class="section-mechanics__main-btns w-100" elem-hide>
        <a href="{{ Route('index') }}#submit" class="btn btn-white-primary w-xs-100 wow fadeIn"
           data-wow-duration="0.8s" data-wow-delay="0.2s" data-wow-offset="30" data-form-contest>{{ __('main.main_Participate') }}</a>
    </div>
@endif