...
class AboutController extends Controller
{
    ...
    {
        $items = AboutAwards::latest();

        $items = Filter::apply([
            'value', 'value_en', 'isVideo'
        ], $request, $items);
    ...
    public function store(Request $request)
    {
        ...
        $post = AboutAwards::create($request->all());
    ...
    {
        return AboutAwards::find($id);
    }

    public function update(Request $request, AboutAwards $post)
    {
        $request = $this->before($request);
    ...
    {
        $post = AboutAwards::find($id);

        if ($post) {
            AboutAwards::destroy($post->id);
        }
