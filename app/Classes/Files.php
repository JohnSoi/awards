...
class Files
{
    ...
    public static $convert_to_webp = true;
    ...
    public static function put($path, $file, $originalName = false)
    {
        if ($originalName) {
            return $file->storeAs(
                $path . '/' . str_replace('.' . $file->extension(), '', $file->hashName()),
                self::substrName(
    ...
    public static function upload($request, $input_name, $path, $thumbs = [], $sharpen = 8)
    {
        if ($request->hasFile($input_name)) {
            ...
            foreach ($files as $file) {
                $file_path = self::put($path, $file, isset($request['originalName']));
